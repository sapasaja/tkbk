...
class Model_reseller extends CI_model{
    ...
    function produk_perkategori($id_reseller,$id_produk_perusahaan,$id_kategori_produk,$limit){
        // Jika id_reseller = 0, tampilkan semua produk (termasuk dari perusahaan)
        if ($id_reseller == 0) {
            return $this->db->query("SELECT a.*, b.nama_reseller, c.nama_kota, d.diskon FROM rb_produk a 
                                        ...
                                        where a.id_produk_perusahaan='$id_produk_perusahaan' AND a.id_kategori_produk='$id_kategori_produk' 
                                        ORDER BY a.id_produk DESC LIMIT $limit");
        } else {
            // Jika id_reseller != 0, tampilkan produk dari reseller lain (bukan dari reseller yang sedang login)
            return $this->db->query("SELECT a.*, b.nama_reseller, c.nama_kota, d.diskon FROM rb_produk a 
                                        LEFT JOIN rb_reseller b ON a.id_reseller=b.id_reseller
                                        LEFT JOIN rb_kota c ON b.kota_id=c.kota_id 
                                        LEFT JOIN rb_produk_diskon d ON a.id_produk=d.id_produk
                                        where a.id_reseller!='$id_reseller' AND a.id_produk_perusahaan='$id_produk_perusahaan' AND a.id_kategori_produk='$id_kategori_produk' 
                                        ORDER BY a.id_produk DESC LIMIT $limit");
        }
    }
