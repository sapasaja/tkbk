...
class Produk extends CI_Controller {
	...
	function keranjang(){
		...
		if ($id_produk!=''){
			// No stock check - all products are available
			...
			if ($this->session->idp == ''){
				...
				if ($this->session->reseller==$id_reseller){
					if ($cek >=1){
						$this->db->query("UPDATE rb_penjualan_temp SET jumlah=jumlah+".(int)$qty." where session='".$this->session->idp."' AND id_produk='$id_produk'");
					}else{
						...
						$data = array('session'=>$this->session->idp,
					        		  'id_produk'=>$id_produk,
					        		  'jumlah'=>(int)$qty,
					        		  'diskon'=>(float)$disk['diskon'],
					        		  'harga_jual'=>(float)$harga_konsumen,
					        		  'satuan'=>$harga['satuan'],
	...
	function kurirdata(){
		...
		// Ambil daftar kurir yang diizinkan dari config
		$allowed_couriers = $this->config->item('rajaongkir_allowed_couriers', 'rajaongkir');
		
		// Jika ada kurir spesifik yang dipilih
		if (!empty($kurir)) {
			// Cek apakah kurir yang dipilih diizinkan
			if (empty($allowed_couriers) || in_array($kurir, $allowed_couriers)) {
				$result = $this->rajaongkir_v2->cost($iden['kota_id'], $this->input->get('kota'), $berat, $kurir);
				$data['data'] = json_decode($result, true);
			} else {
				$data['data'] = array(); // Kurir tidak diizinkan
			}
		} else {
			// Jika tidak ada kurir spesifik, ambil semua kurir yang diizinkan
			if (empty($allowed_couriers)) {
				// Tampilkan semua kurir jika tidak ada filter
				$result = $this->rajaongkir_v2->cost($iden['kota_id'], $this->input->get('kota'), $berat, '');
				$data['data'] = json_decode($result, true);
			} else {
				// Ambil data untuk setiap kurir yang diizinkan
				$all_data = array();
				foreach ($allowed_couriers as $courier) {
					$result = $this->rajaongkir_v2->cost($iden['kota_id'], $this->input->get('kota'), $berat, $courier);
					$courier_data = json_decode($result, true);
					if (!empty($courier_data) && isset($courier_data['data']['results'])) {
						$all_data = array_merge($all_data, $courier_data['data']['results']);
					}
				}
				$data['data'] = $all_data;
			}
		...
		if (isset($_POST['submit'])){
				if ($this->session->idp!=''){
					...
					foreach ($keranjang->result_array() as $row) {
						...
						// Cek apakah produk digital, jika ya berikan akses
						$digital_check = $this->db->query("SELECT * FROM rb_produk_digital WHERE id_produk='$row[id_produk]' AND status='aktif'")->row_array();
						if ($digital_check) {
							$data_akses = array('id_penjualan'=>$idp,
											   'id_produk_digital'=>$digital_check['id_produk_digital'],
											   'id_konsumen'=>$id,
					...
					$message      = "<html><body>Halooo! <b>".$this->input->post('a')."</b> ... <br> Hari ini pada tanggal <span style='color:red'>$tglaktif</span> Anda telah order produk di $iden[nama_website].
						...
						<table style='width:100%;' class='table table-striped'>
					          ...
					          $digital_info = $this->db->query("SELECT * FROM rb_produk_digital WHERE id_produk='$row[id_produk]' AND status='aktif'")->row_array();
					          if ($digital_info) {
					              $berat_info = "Produk Digital";
					              $akses_info = "<br><small style='color:green'>✓ Akses ebook langsung tersedia</small>";
					          } else {
					              $berat_info = ($row['berat']*$row['jumlah'])." Gram";
					              $akses_info = "";
					          }
					          
					$message .= "<tr bgcolor='#f5f5f5'><td>$no</td>
					                    <td>$row[nama_produk]$akses_info</td>
					          ...
					          $digital_count = $this->db->query("SELECT COUNT(*) as digital_count FROM rb_penjualan_detail a 
					                                            JOIN rb_produk_digital pd ON a.id_produk = pd.id_produk 
					                                            WHERE a.id_penjualan='".$idp."' AND pd.status='aktif'")->row_array();
					          
					          if ($digital_count['digital_count'] > 0) {
					              $berat_info = "Produk Digital - Tidak Perlu Pengiriman";
					              $berat_color = "lightgreen";
					          } else {
					              $berat_info = "$total[total_berat] Gram";
					              $berat_color = "lightblue";
					          }
					...
					$message .= "<tr bgcolor='lightgreen'>
					                ...
					                <tr bgcolor='$berat_color'>
					                  <td colspan='5'><b>Total Berat</b></td>
					                  <td><b>$berat_info</b></td>
					                </tr>
	...
	public function detail(){
		...
		$dat = $this->db->query("SELECT * FROM rb_produk where produk_seo='$ids'");
	    $row = $dat->row();
	    $total = $dat->num_rows();
	        if ($total == 0){
	        	redirect('main');
		...
		// Load model flash sale untuk cek harga flash sale
		$this->load->model('model_flash_sale');
		$data['flash_sale'] = $this->model_flash_sale->is_product_in_flash_sale($row->id_produk);
