...
class Administrator extends CI_Controller {
	function index(){
		if (isset($_POST['submit'])){
			...
			if ($total > 0){
				...
				$redirect_url = $this->uri->segment(1).'/home';
            ...
            if ($this->session->level!=''){
              // Debug: Log the redirect URL
              $redirect_url = $this->uri->segment(1).'/home';
              // echo "Already logged in, redirecting to: " . $redirect_url; // Uncomment for debugging
              redirect($redirect_url);
            }else{
    ...
    // Controller Modul sponsor Atas

    function sponsoratas(){
        cek_session_akses('sponsoratas',$this->session->id_session);
        if ($this->session->level=='admin'){
            $data['record'] = $this->model_app->view_ordering('sponsoratas','id_sponsoratas','DESC');
        }else{
            $data['record'] = $this->model_app->view_where_ordering('sponsoratas',array('username'=>$this->session->username),'id_sponsoratas','DESC');
        }
        $this->template->load('administrator/template','administrator/mod_sponsoratas/view_sponsoratas',$data);
    }

    function tambah_sponsoratas(){
        cek_session_akses('sponsoratas',$this->session->id_session);
        if (isset($_POST['submit'])){
            $config['upload_path'] = 'asset/foto_sponsoratas/';
            ...
            $this->model_app->insert('sponsoratas',$data);  
            redirect($this->uri->segment(1).'/sponsoratas');
        }else{
            $this->template->load('administrator/template','administrator/mod_sponsoratas/view_sponsoratas_tambah');
        }
    }

    function edit_sponsoratas(){
        cek_session_akses('sponsoratas',$this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])){
            $config['upload_path'] = 'asset/foto_sponsoratas/';
            ...
            $where = array('id_sponsoratas' => $this->input->post('id'));
            $this->model_app->update('sponsoratas', $data, $where);
            redirect($this->uri->segment(1).'/sponsoratas');
        }else{
            if ($this->session->level=='admin'){
                $proses = $this->model_app->edit('sponsoratas', array('id_sponsoratas' => $id))->row_array();
            }else{
                $proses = $this->model_app->edit('sponsoratas', array('id_sponsoratas' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses);
            $this->template->load('administrator/template','administrator/mod_sponsoratas/view_sponsoratas_edit',$data);
        }
    }

    function delete_sponsoratas(){
        cek_session_akses('sponsoratas',$this->session->id_session);
        if ($this->session->level=='admin'){
            $id = array('id_sponsoratas' => $this->uri->segment(3));
        }else{
            $id = array('id_sponsoratas' => $this->uri->segment(3), 'username'=>$this->session->username);
        }
        $this->model_app->delete('sponsoratas',$id);
        redirect($this->uri->segment(1).'/sponsoratas');
    }


	// Controller Modul sponsor Home

	function sponsorhome(){
		cek_session_akses('sponsorhome',$this->session->id_session);
        if ($this->session->level=='admin'){
            $data['record'] = $this->model_app->view_ordering('sponsortengah','id_sponsortengah','DESC');
        }else{
            $data['record'] = $this->model_app->view_where_ordering('sponsortengah',array('username'=>$this->session->username),'id_sponsortengah','DESC');
        }
		$this->template->load('administrator/template','administrator/mod_sponsorhome/view_sponsorhome',$data);
	}

	function tambah_sponsorhome(){
		cek_session_akses('sponsorhome',$this->session->id_session);
		if (isset($_POST['submit'])){
    		$config['upload_path'] = 'asset/foto_sponsortengah/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG|swf';
            $config['max_size'] = '3000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('c');
            ...
            $this->model_app->insert('sponsortengah',$data);  
			redirect($this->uri->segment(1).'/sponsorhome');
		}else{
			$this->template->load('administrator/template','administrator/mod_sponsorhome/view_sponsorhome_tambah');
		}
	}

	function edit_sponsorhome(){
		cek_session_akses('sponsorhome',$this->session->id_session);
		$id = $this->uri->segment(3);
		if (isset($_POST['submit'])){
			$config['upload_path'] = 'asset/foto_sponsortengah/';
            ...
            $where = array('id_sponsortengah' => $this->input->post('id'));
            $this->model_app->update('sponsortengah', $data, $where);
			redirect($this->uri->segment(1).'/sponsorhome');
		}else{
            if ($this->session->level=='admin'){
                $proses = $this->model_app->edit('sponsortengah', array('id_sponsortengah' => $id))->row_array();
            }else{
                $proses = $this->model_app->edit('sponsortengah', array('id_sponsortengah' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses);
			$this->template->load('administrator/template','administrator/mod_sponsorhome/view_sponsorhome_edit',$data);
		}
	}

	function delete_sponsorhome(){
        cek_session_akses('sponsorhome',$this->session->id_session);
		if ($this->session->level=='admin'){
            $id = array('id_sponsortengah' => $this->uri->segment(3));
        }else{
            $id = array('id_sponsortengah' => $this->uri->segment(3), 'username'=>$this->session->username);
        }
        $this->model_app->delete('sponsortengah',$id);
		redirect($this->uri->segment(1).'/sponsorhome');
	}


    // Controller Modul sponsor Sidebar

    function sponsorsidebar(){
        cek_session_akses('sponsorsidebar',$this->session->id_session);
        if ($this->session->level=='admin'){
            $data['record'] = $this->model_app->view_ordering('pasangsponsor','id_pasangsponsor','DESC');
        }else{
            $data['record'] = $this->model_app->view_where_ordering('pasangsponsor',array('username'=>$this->session->username),'id_pasangsponsor','DESC');
        }
        $this->template->load('administrator/template','administrator/mod_sponsorsidebar/view_sponsorsidebar',$data);
    }

    function tambah_sponsorsidebar(){
        cek_session_akses('sponsorsidebar',$this->session->id_session);
        if (isset($_POST['submit'])){
            $config['upload_path'] = 'asset/foto_pasangsponsor/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG|swf';
            $config['max_size'] = '3000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('c');
            ...
            $this->model_app->insert('pasangsponsor',$data);
            redirect($this->uri->segment(1).'/sponsorsidebar');
        }else{
            $this->template->load('administrator/template','administrator/mod_sponsorsidebar/view_sponsorsidebar_tambah');
        }
    }

    function edit_sponsorsidebar(){
        cek_session_akses('sponsorsidebar',$this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])){
            $config['upload_path'] = 'asset/foto_pasangsponsor/';
            ...
            $where = array('id_pasangsponsor' => $this->input->post('id'));
            $this->model_app->update('pasangsponsor', $data, $where);
            redirect($this->uri->segment(1).'/sponsorsidebar');
        }else{
            if ($this->session->level=='admin'){
                $proses = $this->model_app->edit('pasangsponsor', array('id_pasangsponsor' => $id))->row_array();
            }else{
                $proses = $this->model_app->edit('pasangsponsor', array('id_pasangsponsor' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses);
            $this->template->load('administrator/template','administrator/mod_sponsorsidebar/view_sponsorsidebar_edit',$data);
        }
    }

    function delete_sponsorsidebar(){
        cek_session_akses('sponsorsidebar',$this->session->id_session);
        if ($this->session->level=='admin'){
            $id = array('id_pasangsponsor' => $this->uri->segment(3));
        }else{
            $id = array('id_pasangsponsor' => $this->uri->segment(3), 'username'=>$this->session->username);
        }
        $this->model_app->delete('pasangsponsor',$id);
        redirect($this->uri->segment(1).'/sponsorsidebar');
    }
    ...
    function import_produk(){
        ...
        $this->template->load('administrator/template','administrator/additional/mod_produk/view_import_produk',$data);
    }
    ...
    function download_template($type = 'physical') {
        ...
        if ($type == 'digital') {
            ...
            $filename = 'template_produk_digital.csv';
        } else {
            $template = $this->excel_import->generate_physical_template();
            $filename = 'template_produk_fisik.csv';
        }
...
function delete_testimoni(){
	...
	function produk_digital(){
		cek_session_akses('produk',$this->session->id_session);
		$data['record'] = $this->model_app->view_join_where('rb_produk_digital','rb_produk','id_produk', array('rb_produk_digital.status' => 'aktif'),'id_produk_digital','DESC');
		$this->template->load('administrator/template','administrator/additional/mod_produk_digital/view_produk_digital',$data);
	}

	// Function to check and set upload limits for large files
	private function set_upload_limits() {
		// Note: Upload limits are set via .htaccess file
		// These ini_set calls are kept for compatibility but may not work on all servers
	...
	function tambah_produk_digital(){
		cek_session_akses('produk',$this->session->id_session);
		if (isset($_POST['submit'])){
			// Set upload limits for large files
			$this->set_upload_limits();
			...
			$data_produk = array(
				...
				'produk_seo' => seo_title($this->input->post('b')),
				'satuan' => $this->db->escape_str($this->input->post('c')),
				'harga_beli' => $this->input->post('d'),
				'harga_reseller' => $this->input->post('e'),
				'harga_konsumen' => $this->input->post('f'),
				'keterangan' => $this->db->escape_str($this->input->post('ff')),
				'gambar' => $gambar,
				'id_reseller' => '0', // Company product
				'is_digital' => 'Y',
				'waktu_input' => date('Y-m-d H:i:s')
			);
			...
			// Now handle digital product file upload
			$config2['upload_path'] = 'asset/files/ebook/';
			$config2['allowed_types'] = 'pdf|doc|docx|epub|mobi';
			$config2['max_size'] = '1048576'; // 1GB (1024*1024*1024 bytes)
			$this->upload->initialize($config2);
			
			if ($this->upload->do_upload('file_ebook')){
				$hasil = $this->upload->data();
				$file_size = $hasil['file_size'];
				$file_ext = $hasil['file_ext'];
				
							// Upload preview jika ada
			$preview_file = '';
			if (isset($_FILES['preview_file']) && $_FILES['preview_file']['name'] != '') {
					$config3['upload_path'] = 'asset/files/ebook/preview/';
					$config3['allowed_types'] = 'pdf|jpg|jpeg|png';
					$config3['max_size'] = '5000'; // 5MB
					$this->upload->initialize($config3);
					if ($this->upload->do_upload('preview_file')) {
						$preview = $this->upload->data();
						$preview_file = $preview['file_name'];
					}
				}

				$data_digital = array(
					'id_produk' => $id_produk,
					'nama_ebook' => $this->db->escape_str($this->input->post('b')), // Use product name as ebook name
					'deskripsi' => $this->db->escape_str($this->input->post('ff')), // Use product description as ebook description
					'file_ebook' => $hasil['file_name'],
					'ukuran_file' => $this->formatBytes($file_size),
					'format_file' => str_replace('.', '', $file_ext),
				...
				redirect('administrator/produk_digital');
			}else{
				echo $this->upload->display_errors();
			}
		}else{
			$data['kategori'] = $this->model_app->view_ordering('rb_kategori_produk','nama_kategori','ASC');
			$this->template->load('administrator/template','administrator/additional/mod_produk_digital/view_produk_digital_tambah',$data);
		}
	}

	function edit_produk_digital(){
		cek_session_akses('produk',$this->session->id_session);
		$id = $this->uri->segment(3);
		if (isset($_POST['submit'])){
			// Set upload limits for large files
			...
			if (isset($_FILES['userfile']) && $_FILES['userfile']['name'] != '') {
				if ($this->upload->do_upload('userfile')){
					$hasil = $this->upload->data();
					$data_produk['gambar'] = $hasil['file_name'];
				}
			}
			...
			// Update discount
			$cek_diskon = $this->model_app->view_where("rb_produk_diskon",array('id_produk'=>$this->input->post('id_produk')))->row_array();
			if ($cek_diskon) {
				$data_diskon = array('diskon' => $this->input->post('diskon'));
				$this->model_app->update('rb_produk_diskon', $data_diskon, array('id_produk' => $this->input->post('id_produk')));
			} else {
				if ($this->input->post('diskon') > 0) {
					$data_diskon = array(
			...
			if (isset($_FILES['file_ebook']) && $_FILES['file_ebook']['name'] != '') {
				$config2['upload_path'] = 'asset/files/ebook/';
				$config2['allowed_types'] = 'pdf|doc|docx|epub|mobi';
				$config2['max_size'] = '1048576'; // 1GB (1024*1024*1024 bytes)
				$this->upload->initialize($config2);
				...
				if ($this->upload->do_upload('file_ebook')){
					$hasil = $this->upload->data();
					$data_digital['file_ebook'] = $hasil['file_name'];
					$data_digital['ukuran_file'] = $this->formatBytes($hasil['file_size']);
					$data_digital['format_file'] = str_replace('.', '', $hasil['file_ext']);
				}
			}
			
			// Upload preview baru jika ada
			if (isset($_FILES['preview_file']) && $_FILES['preview_file']['name'] != '') {
				$config3['upload_path'] = 'asset/files/ebook/preview/';
				$config3['allowed_types'] = 'pdf|jpg|jpeg|png';
				$config3['max_size'] = '5000'; // 5MB
				...
				if ($this->upload->do_upload('preview_file')) {
					$preview = $this->upload->data();
					$data_digital['preview_file'] = $preview['file_name'];
				}
			}
			
			$where = array('id_produk_digital' => $id);
			$this->model_app->update('rb_produk_digital', $data_digital, $where);
			redirect('administrator/produk_digital');
		}else{
			$proses = $this->model_app->edit('rb_produk_digital', array('id_produk_digital' => $id))->row_array();
			$produk = $this->model_app->edit('rb_produk', array('id_produk' => $proses['id_produk']))->row_array();
			$data = array(
				'rows' => array_merge($proses, $produk),
				'kategori' => $this->model_app->view_ordering('rb_kategori_produk','nama_kategori','ASC')
			);
			$this->template->load('administrator/template','administrator/additional/mod_produk_digital/view_produk_digital_edit',$data);
		}
	}

	function delete_produk_digital(){
		cek_session_akses('produk',$this->session->id_session);
		$id = $this->uri->segment(3);
		
		// Ambil data produk digital
		$digital = $this->model_app->edit('rb_produk_digital', array('id_produk_digital' => $id))->row_array();
		
		// Hapus file fisik
		if ($digital['file_ebook'] != '') {
			unlink('asset/files/ebook/'.$digital['file_ebook']);
		}
		if ($digital['preview_file'] != '') {
			unlink('asset/files/ebook/preview/'.$digital['preview_file']);
		}
		
		// Update produk menjadi non-digital
		$update_produk = array('is_digital' => 'N');
		$where_produk = array('id_produk' => $digital['id_produk']);
		$this->model_app->update('rb_produk', $update_produk, $where_produk);
		
		// Hapus data digital
		$where = array('id_produk_digital' => $id);
		$this->model_app->delete('rb_produk_digital',$where);
		redirect('administrator/produk_digital');
	}

	function akses_ebook(){
		cek_session_akses('produk',$this->session->id_session);
		$data['record'] = $this->db->query("
			SELECT ae.*, pd.nama_ebook, k.nama_lengkap as nama_konsumen 
			FROM rb_akses_ebook ae 
			JOIN rb_produk_digital pd ON ae.id_produk_digital = pd.id_produk_digital 
			LEFT JOIN rb_konsumen k ON ae.id_konsumen = k.id_konsumen 
			ORDER BY ae.id_akses DESC
		")->result_array();
		$this->template->load('administrator/template','administrator/additional/mod_produk_digital/view_akses_ebook',$data);
	}

	function download_log(){
		cek_session_akses('produk',$this->session->id_session);
		$data['record'] = $this->db->query("
			SELECT dl.*, pd.nama_ebook, k.nama_lengkap as nama_konsumen 
			FROM rb_download_log dl 
			JOIN rb_akses_ebook ae ON dl.id_akses = ae.id_akses 
			JOIN rb_produk_digital pd ON ae.id_produk_digital = pd.id_produk_digital 
			LEFT JOIN rb_konsumen k ON ae.id_konsumen = k.id_konsumen 
			ORDER BY dl.id_log DESC
		")->result_array();
		$this->template->load('administrator/template','administrator/additional/mod_produk_digital/view_download_log',$data);
	}
    ...
    function flash_sale(){
        cek_session_akses('flash_sale',$this->session->id_session);
        $data['record'] = $this->db->query("SELECT fs.*, p.nama_produk, p.harga_konsumen, p.gambar 
                                           FROM rb_flash_sale fs 
                                           JOIN rb_produk p ON fs.id_produk = p.id_produk 
                                           ORDER BY fs.id_flash_sale DESC");
        $this->template->load('administrator/template','administrator/additional/mod_flash_sale/view_flash_sale',$data);
    }

    function tambah_flash_sale(){
        cek_session_akses('flash_sale',$this->session->id_session);
        if (isset($_POST['submit'])){
            // Auto-calculate flash sale price if discount percentage is provided
            $id_produk = $this->input->post('id_produk');
            $diskon_persen = $this->input->post('diskon_persen');
            $harga_flash_sale = $this->input->post('harga_flash_sale');
            
            // Get original price
            $produk = $this->model_app->edit('rb_produk', array('id_produk' => $id_produk))->row_array();
            $harga_asli = $produk['harga_konsumen'];
            
            // If flash sale price is not set but discount percentage is, calculate it
            if (empty($harga_flash_sale) && !empty($diskon_persen)) {
                $harga_flash_sale = $harga_asli - (($diskon_persen / 100) * $harga_asli);
            }
            
            // Recalculate discount percentage based on final prices
            $diskon_persen = round((($harga_asli - $harga_flash_sale) / $harga_asli) * 100);
            ...
            $this->model_app->insert('rb_flash_sale',$data);
            redirect('administrator/flash_sale');
        }else{
            $data['produk'] = $this->model_app->view_ordering('rb_produk','id_produk','DESC');
            $this->template->load('administrator/template','administrator/additional/mod_flash_sale/view_flash_sale_tambah',$data);
        }
    }

    function edit_flash_sale(){
        cek_session_akses('flash_sale',$this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])){
            ...
            $where = array('id_flash_sale' => $this->input->post('id_flash_sale'));
            $this->model_app->update('rb_flash_sale', $data, $where);
            redirect('administrator/flash_sale');
        }else{
            $data['produk'] = $this->model_app->view_ordering('rb_produk','id_produk','DESC');
            $data['rows'] = $this->model_app->edit('rb_flash_sale',array('id_flash_sale'=>$id))->row_array();
            $this->template->load('administrator/template','administrator/additional/mod_flash_sale/view_flash_sale_edit',$data);
        }
    }

    function delete_flash_sale(){
        cek_session_akses('flash_sale',$this->session->id_session);
        $id = array('id_flash_sale' => $this->uri->segment(3));
        $this->model_app->delete('rb_flash_sale',$id);
        redirect('administrator/flash_sale');
    }
}
