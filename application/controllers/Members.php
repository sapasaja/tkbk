		...
		if ($id_produk!=''){
			// No stock check - all products are available
				...
				if ($reseller['id_penjual']==$id_reseller){
					if ($cek >=1){
						$this->db->query("UPDATE rb_penjualan_detail SET jumlah=jumlah+".(int)$qty." where id_penjualan='".$this->session->idp."' AND id_produk='$id_produk'");
					}else{
	                    ...
	                    $harga_konsumen = (float)$harga['harga_konsumen']-(float)$disk['diskon'];
